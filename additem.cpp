...
void AddItem::on_btnAdd_clicked()
{
    ...
    query.prepare("insert into inventory_items(Name, Quantity, Status, DOP, DOS, SellingPrice, purchasePrice) values ('" +sName +"','" +sQuantity + "','"+ sStatus + "','" +sDOP + "','"+ sDOS+ "','"+sSellingPrice+ "','"+ sPurchasePrice + "')");
...
void AddItem::on_btnAdd_2_clicked()
{
    QWidget::close();
}
